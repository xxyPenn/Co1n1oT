...
static bool Runbinfile(char* binary_file_name);
...
char flag_A[] = "0:FlagA.txt"; //test A file name txt and HTTP_DownloadFileTransaction flag in WifiHandler.c
char flag_B[] = "0:FlagB.txt"; //test B file name txt
char bin_A[] = "0:TestA.bin"; //test A file name bin
char bin_B[] = "0:TestB.bin"; //test A file name bin
char fw_flag[] = "0:fw.txt"; // customized firmware downloaded flag
char fw_file[] = "0:TestA_a10.bin"; // firmware update binary file name
char helpStr[64]; //Used to help print values
bool isTestA = false; // flag to see if we should load testA
bool loadFw = false;  // flag to indicate if we should load FW into NVM
...
int main(void)
{
	  ...
	  /*3.) STARTS BOOTLOADER HERE!*/
			...
			//Only load binary files and clear memory spaces if seen flag(s)
			res = f_open(&file_object, (char const *)fw_flag, FA_READ);
			if (res == FR_OK)
			{
				loadFw = true;
				f_unlink(fw_flag); //delete customized flag after checking it.
				f_unlink(flag_A); //delete FlagA after checking it.
				//erase first
				...
				// Clear the MCU memory space
				for(int i = 0; i < (parameters.nvm_number_of_pages / PAGE_PER_ROW) - BOOTLOADER_ROW_NUM; i++)
				{
					// Iterating erasing a single row until erases all rows
			...
			// flag not detected, go to application code to download fw
			else
			{
				SerialConsoleWriteString("jumping to app");
				goto jmp_to_app;
			}
			// should only enter here when flag detected and addr space cleared
			if(loadFw){
				fw_file[0] = LUN_ID_SD_MMC_0_MEM + '0';
				if(!Runbinfile(fw_file)){goto fw_update_error;}
				f_unlink(fw_file);
				f_unlink(flag_B);
			}
			#endif
			...
			//read SD card file
			flag_A[0] = LUN_ID_SD_MMC_0_MEM + '0';
			flag_B[0] = LUN_ID_SD_MMC_0_MEM + '0';
			...
			if(isTestA){
				bin_A[0] = LUN_ID_SD_MMC_0_MEM + '0';
				if(!Runbinfile(bin_A)){goto fw_update_error;}
				f_unlink(flag_A);//delete FlagA after creating it.
			}
			else
			{	
				bin_B[0] = LUN_ID_SD_MMC_0_MEM + '0';
				if(!Runbinfile(bin_B)){goto fw_update_error;}
				f_unlink(flag_B);//delete FlagB after creating it.
			}

			#endif

//example here not used
			#ifdef MEM_EXAMPLE
			//Example
...
******************************************************************************/
// Flashes the bin file into the NVM
static bool Runbinfile(char* binary_file_name)
{
		// CRC check results for SD card and NVM
		uint32_t resultCrcSd = 0;
		uint32_t resultCrcNVM = 0;
		...
		while(numBytesRead == 256){
			res = f_read(&file_object_bin, &readBuffer[0], 256, &numBytesRead);
			if(res != FR_OK)
			{
			...
			// write to nvm buffer
			res = nvm_write_buffer(APP_START_ADDRESS + (i*ROW_SIZE), &readBuffer[0], 64);
			res = nvm_write_buffer(APP_START_ADDRESS + (i*ROW_SIZE) + 64, &readBuffer[64], 64);
			res = nvm_write_buffer(APP_START_ADDRESS +(i*ROW_SIZE) + 128, &readBuffer[128], 64);
			res = nvm_write_buffer(APP_START_ADDRESS + (i*ROW_SIZE) + 192, &readBuffer[192], 64);
			if (res != FR_OK)
			{
				SerialConsoleWriteString("Test write to NVM failed!\r\n");
			}
			...
			//calculate SD CRC chunk ->  See http://ww1.microchip.com/downloads/en/DeviceDoc/SAM-D21-Family-Silicon-Errata-and-DataSheet-Clarification-DS80000760D.pdf Section 1.8.3
			*((volatile unsigned int*) 0x41007058) &= ~0x30000UL;
			dsu_crc32_cal(readBuffer, ROW_SIZE, &resultCrcSd);
			*((volatile unsigned int*) 0x41007058) |= 0x20000UL;
			//calculate NVM CRC chunk
