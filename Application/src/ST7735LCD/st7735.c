...
void lcd_spi_init(void){
	LogMessage(LOG_INFO_LVL, "Initializing LCD Screen...");
	...
	//port_pin_set_config(LCD_0_CD_GPIO, &pin_conf);

	// Initialize SPI interface and enable it
	struct spi_config config;
	spi_get_config_defaults(&config);

	config.mux_setting = LCD_SPI_PINMUX_SETTING;
	config.pinmux_pad0 = LCD_SPI_PINMUX_PAD0;
	config.pinmux_pad1 = LCD_SPI_PINMUX_PAD1;
	config.pinmux_pad2 = LCD_SPI_PINMUX_PAD2;
	config.pinmux_pad3 = LCD_SPI_PINMUX_PAD3;
	config.generator_source = LCD_SPI_SOURCE_CLOCK;
	config.mode_specific.master.baudrate = SDMMC_CLOCK_INIT;

	spi_init(&sd_mmc_master, LCD_SPI, &config);
